...
5. 判断新正文属于叙事、教程步骤、清单、对比、观点或混合结构，并选择最利于手机阅读的排版。
...
- 排版必须由内容关系决定，禁止所有内容套同一种段落模板：叙事和观点按语义转折自然分段；教程和流程将每个步骤单独成行；清单和对比按项目或维度分行；短内容可以少分段。
- 较长正文不能堆成一个连续段落；使用换行形成可扫描的信息层级，但不要为了凑段数拆碎完整句意，不使用 Markdown 表格、代码块或多级标题。
- Emoji 只在有助于扫读时使用，不堆砌；避免“绝了、封神、听我一句劝”等机械套话。
- 标签输出 3 至 5 个，必须与主题直接相关。
"""

DEFAULT_CREATIVE_PROMPT = """请忠实复刻原文的选题价值、受众痛点、信息密度和阅读节奏，同时换一个新的切入角度。正文必须具体、可执行、自然，避免营销腔和空话，并根据叙事、步骤、清单、对比等实际内容关系智能排版。"""

OUTPUT_SCHEMA_PROMPT = """只返回一个 JSON 对象，不要返回 Markdown 代码块、分析过程或额外说明：
{
  "ai_title": "15至20个可见字符的标题",
  "ai_content": "根据内容结构智能排版并使用换行分层的完整正文",
  "ai_tags": ["#话题1", "#话题2", "#话题3"]
}"""
...
def _validate_result(
    ...
    elif _visible_length(content) >= 120 and "\n" not in content:
        errors.append("ai_content 较长但没有任何换行，必须根据内容关系智能排版")
    elif any(_visible_length(line) > 180 for line in content.splitlines()):
        errors.append("ai_content 存在超过 180 字的连续长段，需要按语义层级重新排版")
